...
log_info("Retornando os dados:");
log_trace("getPos ->");
  Node *aux = getNodeByPos(list,pos);
  if (aux==NULL)
  log_debug("DATA: %p", aux);
  log_warning("Nao ha dados!!!");
    return NULL;
  else
  log_debug("DATA: %p", aux);
  log_trace("getPos <-");
...
Node* getNodeByPos(LinkedList *list, int pos) {
    log_info("Buscando a localizacao desejada do no");
    log_trace("getNodeBypos ->");
        if(isEmpty(list) || pos>=list->size) return NULL;
    node *aux = list->first;
    log_debug("node: %p", first);
        for (int count = 0;(aux!=NULL && count<pos); count++, aux = aux ->next);
    log_debug("Count: %d, aux: %p", count, aux);
       if(aux == NULL){
        log_warning("O endereco de memoria do no nao foi localizado");
        return NULL;
    }
    log_trace("getNodeByPos <-");
    return aux;
